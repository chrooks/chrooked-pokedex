...
- [x] Milestone 2 (Slice 2) — CRUD for all five kinds. **Slice 2b done** (2026-06-13): type-chart + behaviors CRUD, and species learnset/evolution editing — completing the milestone. Backend: a new `behavior_yaml` renderer (the seed never writes behaviors, so this is their only writer; block-style, matches the hand-authored format) and a `type_chart_yaml` extraction; `web/crud.replace_type_chart` (whole-list write — the type chart is one file) and `upsert_behavior`/`delete_behavior` (no citation guard — human-owned, no dangling data ref); `aka` added to the behavior serializer so an edit can't strip the engine symbol. Routes: `PUT /api/type-chart`, `PUT/DELETE /api/behaviors/{id}`. Frontend: `TypeChartTab` became an inline whole-list editor (add/remove rows, multiplier select, dirty-tracked Save); a `BehaviorEditor` dialog with the structured form (applies-to, repeatable effect rows with a closed-set trigger select, test cases, notes, engine hints); `SpeciesEditor` gained learnset (repeatable level/move, overrides-only vs base) and evolution (from + method key/value) editing. Stable row ids (`lib/rowId`) on every repeatable list. 250 Python tests green (10 new); tsc + ESLint + vite build clean; runtime smoke confirmed (overrides-only species writes, block-style behavior YAML, 422 on bad trigger/multiplier). Reviews: python + react + a11y fan-out; all HIGH fixed (one react "HIGH" rejected as a misread — see Decision Log).

  - **Slice 2a done** (2026-06-13): CRUD for the three simple-record kinds — species, owned moves, owned abilities. Backend `web/crud.py`: a validate-then-write pipeline (stage in a temp dir inside the target, reload through the loader, atomic `os.replace`, return the validated object) with `ValidationError`→422 (loader's verbatim message, writes nothing), `CitationError`→409 (delete-guard naming citing species; `?confirm=true` overrides), `NotFoundError`→404; unknown top-level fields rejected on all three kinds; `aka` added to the move/ability serializers so an edit can't strip the engine symbol. Routes: `GET/PUT/DELETE /api/species/{id}` (GET returns the **raw** Override, overrides-only) + `PUT/DELETE` for moves/abilities. Writer `_move_yaml`/`_ability_yaml` promoted public. Frontend: `useResource.reload()`, a `useSubmit` hook, `api.ts` mutations with a 409-aware `ApiError`, a self-trapping `EditorDialog`, typed `fields.tsx`; a `SpeciesEditor` in the detail ledger (edits stats/types/abilities **overrides-only** by diffing against base, round-trips learnset/evolution/aka untouched; "Revert to base" deletes the Override); `MoveEditor`/`AbilityEditor` dialogs in the tabs (create/edit/delete, full-record round-trip, 409 "Delete anyway" confirm); a standing `ReseedNote` in the rail on the machine-owned kinds. 240 Python tests green (24 new); tsc + ESLint + vite build clean; runtime smoke against a copy of the real ruleset confirmed overrides-only writes + the 409 guard. Reviews: python + react + a11y fan-out; all HIGH/CRITICAL fixed. (Type-chart + behaviors CRUD and species learnset/evolution editing followed in slice 2b.)
...
- Decision (M2b): the type chart is written as a **whole-list replace** (`PUT /api/type-chart` with the full override list), not per-entry CRUD.
  Rationale: the type chart is one file (`type-chart/overrides.yaml`) holding a list — there's no per-entry id to address. The inline editor sends the whole working list; the loader validates it; delete is just "send the list without that row." Simplest model that fits the on-disk shape.
  Date/Author: 2026-06-13, Claude (implementation).

- Decision (M2b): behaviors get a new YAML renderer in `seed/writer.py` even though the seed never writes them.
  Rationale: behaviors are human-owned and were previously only hand-authored, so no writer existed. The UI now needs one. It lives beside the other renderers (cohesion) and matches the hand-authored block-style format so a UI-written spec is indistinguishable from a hand-written one. `delete_behavior` has no citation guard: a behavior attaches by `chrooked_id` and leaves no dangling *data* reference when removed.
  Date/Author: 2026-06-13, Claude (implementation).

- Decision (M2b): the species editor initializes its learnset form from the **merged** value (`entry.learnset`) and diffs against base on save — deliberately, despite a review flagging it.
  Rationale: a review HIGH suggested initializing from base instead. That is the wrong fix: a species *with* a learnset Override, opened and saved untouched, would then diff base-vs-base, drop the Override, and lose the learnset (data loss). Init-from-merged + diff-against-base means an untouched Override is faithfully re-emitted (preserved) while an untouched non-overridden species emits nothing. The "cost" is re-writing identical learnset YAML on an unrelated edit — idempotent, not a bug. The same logic governs every overrides-only field.
  Date/Author: 2026-06-13, Chris + Claude (review triage).

- Decision (M2b): repeatable editor rows carry a stable session-local id (`lib/rowId`) used as the React `key`, not the array index.
  Rationale: index keys reattach DOM/focus state to the wrong row when a middle row is removed. A monotonic counter is enough — ids are never persisted (stripped before the API call). Applied across type-chart, behavior (effects/tests/notes/hints), and species learnset/evolution rows.
  Date/Author: 2026-06-13, Claude (implementation, from review).
...
Milestone 2b review fan-out (2026-06-13): `python-reviewer` (APPROVE) + `react-reviewer` + `a11y-architect` on the 2b diff. All actionable HIGH fixed; one react "HIGH" rejected.

### M2b — Fixed

- **[react HIGH] Index keys on removable rows.** Every repeatable list (type-chart, behavior effects/tests/notes/hints, species learnset/evolution) now keys by a stable `lib/rowId`, not the array index.
- **[python MEDIUM] Dead `isinstance(error, ValidationError)` guards** in `replace_type_chart`/`upsert_behavior` removed (ValidationError isn't a ValueError, so it already propagates) and replaced with the explanatory comment from `upsert_move`.
- **[a11y HIGH] Missing accessible names.** SpeciesEditor `<form>` got `aria-label`; the "Revert to base" and BehaviorEditor "Delete" buttons name their target; every section wired `aria-labelledby` to its heading; add-buttons got `aria-label`s; the `Esc` `<kbd>` no longer leaks into the close button's name (across Behavior/Move/Ability dialogs); the type-chart "vs" separator is `aria-hidden`.
- **[python LOW] `_evolution_flow`** got its `EvolutionOverride` type annotation.

### M2b — Rejected / deferred

- **[react HIGH — REJECTED] "Init species learnset from base, not merged."** The suggested fix causes data loss (drops an existing learnset Override on an untouched save). The current init-from-merged + diff-against-base is correct; see the Decision Log.
- **[react MEDIUM] TypeChartTab seeds working state from fetched data via `useEffect`.** Kept — `reload()` only fires after a successful save, so re-seeding is intended (adopts the canonical server sort); documented inline.
- **[a11y MEDIUM] Disabled "Save changes" has no recovery hint** when nothing is dirty — accepted; the dirty-gate is the intended affordance.
- **[react/a11y] `useMemo` on the dirty check, `useCallback` on `set` helpers, `jsx-a11y` plugin adoption** — parked as before.
