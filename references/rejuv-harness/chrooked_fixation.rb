# chrooked:fixation
# Fixation — special moves deal 1.5x; user locks into its first move
...
CHROOKED_DAMAGE_MODS[:FIXATION] = lambda { |move, attacker, opponent|
  move.pbIsSpecial?(attacker, move.pbType(attacker)) ? 1.5 : 1.0
}
CHROOKED_AFTER_MOVE[:FIXATION] = lambda { |battler, move_symbol, battle|
