...
- **New delivery path:** a PC sort is not an ability, move, or status — the three `applies_to` kinds — and it has no battle trigger, so forcing it into a behavior spec would have meant inventing a fake trigger. Instead `behavior_install.py` grew an `_ENGINE_MODS` list: engine-level mods that ship alongside the core whenever anything installs, validated by the same `# chrooked:<id>` tag and reported just as honestly (absent is silent, untagged is blocked). Three pytest cases cover it.
